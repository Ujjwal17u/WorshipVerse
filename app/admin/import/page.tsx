...
export default function ImportPage() {
  ...
  const confirmImport = async () => {
    ...
    setError(null)
    try {
      const rows = preview.rows.filter((row) => row.status === 'valid' || (duplicateMode === 'update' && row.status === 'duplicate' && row.duplicateOf === 'database'))
      const response = await fetch('/api/songs/import', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ confirm: true, duplicateMode, rows }),
      })
