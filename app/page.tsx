...
// English songs blanked for user to add Hindi songs
const songs: Song[] = [
  {
    id: 1,
    title: '',
...
function AdminPreview() {
  const stats = [{ label: 'Total songs', value: '6' }, { label: 'Praise songs', value: '3' }, { label: 'Worship songs', value: '3' }, { label: 'Favorites', value: '0' }]
  return <div><div className="mb-10 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><span className="text-sm font-semibold uppercase tracking-wider text-accent">Content management</span><h1 className="mt-3 text-4xl text-primary">Admin dashboard</h1><p className="mt-3 text-muted-foreground">Manage your song library and keep lyrics organized.</p></div><button className="flex items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground"><Plus size={17} /> Add new song</button></div><div className="mb-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{stats.map((stat) => <div key={stat.label} className="rounded-xl border border-border bg-card p-5"><p className="text-sm text-muted-foreground">{stat.label}</p><p className="mt-3 text-3xl font-semibold text-primary">{stat.value}</p></div>)}</div><div className="grid gap-6 lg:grid-cols-[1fr_320px]"><section className="rounded-xl border border-border bg-card"><div className="flex items-center justify-between border-b border-border p-5"><div><h2 className="text-lg">Song library</h2><p className="mt-1 text-sm text-muted-foreground">Your current collection</p></div><button className="rounded-lg p-2 text-muted-foreground hover:bg-secondary" aria-label="Settings"><Settings size={18} /></button></div><div className="divide-y divide-border">{songs.map((song) => <div key={song.id} className="flex items-center justify-between gap-4 px-5 py-4"><div className="flex min-w-0 items-center gap-3"><span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-secondary text-primary"><Music2 size={16} /></span><div className="min-w-0"><p className="truncate text-sm font-medium">{song.title}</p><p className="text-xs text-muted-foreground">{song.category} · {song.artist}</p></div></div><button className="text-sm text-muted-foreground hover:text-primary">Edit</button></div>)}</div></section><aside className="flex flex-col gap-4"><div className="rounded-xl border border-border bg-card p-5"><Upload className="mb-4 text-accent" size={23} /><h2 className="text-lg">Import songs</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Upload a CSV file to add multiple songs at once.</p><button className="mt-5 w-full rounded-lg border border-border px-4 py-2.5 text-sm font-medium text-primary hover:bg-secondary">Choose CSV file</button></div><div className="rounded-xl bg-secondary p-5"><FileText className="mb-4 text-primary" size={21} /><h3 className="text-base">CSV format</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">Use columns for title, artist, category, and lyrics.</p></div></aside></div></div>
}
